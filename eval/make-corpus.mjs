...
const MAINTENANCE_LOG_PAGES = [
  ...
  // Appendix B, image-only and therefore invisible to `extractText`.
  //
  // The JPEG is generated by `corpus-assets/make-appendix-b-scan.py`, which is
  // committed for the reason stated at the top of this file: the page's ground
  // truth has to be reviewable as text. Until that script existed the facts the
  // eval scores against — a chiller replaced on 14 March, a Carrier 30XA-1002,
  // a 60-month warranty, and Hartley Mechanical removing the old unit — existed
  // nowhere in source, so grepping the corpus for "chiller" found nothing and
  // `layout-chiller-warranty`'s expected "60" could not be checked without
  // opening the binary. Read `LINES` in that script for the page verbatim.
  //
  // Nothing here runs it: the asset is committed, and regenerating it changes
  // this PDF's bytes. See the script's --force guard.
  {
