...
[Tutorial](docs/tutorial.md), which explains everything from scratch.
...
- It won't make things up. If nothing in your documents matches the
  question, the app says it doesn't know, and the model isn't called at all.
- You only ever see your own documents. That limit is part of the database
  query, so the model can't be talked out of it.
...
All configuration is environment variables, validated at boot, so the app fails
fast on a missing or malformed value instead of at first use. Start from
...
Each block is inert until configured, and none is required to run the app.
...
The full script reference, testing notes and Docker workflow are in
[Usage & Development](docs/usage.md).

For deployment, the Docker stack is served on a Cloudflare domain via
Cloudflare Tunnel, with no open ports, reverse proxy or certificates. `make
...
because refusal accuracy is a hard gate. See
...
Next: if RAG is new to you, read the [Tutorial](docs/tutorial.md). Otherwise
see the [Summary](docs/summary.md) for the whole project on one page.
