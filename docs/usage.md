...
| `AUTH_GITHUB_ID` / `_SECRET` |    –     | Enables GitHub sign-in when both are set. See [OAuth](oauth.md)                                                                |
| `AUTH_GOOGLE_ID` / `_SECRET` |    –     | Enables Google sign-in when both are set. See [OAuth](oauth.md)                                                                |
...
| `RAG_AGENTIC_ENABLED`        |    –     | `true` for the agentic retrieval loop. Default `false`. It is ~10× slower but better on follow-ups; see [RAG](rag.md)          |
| `RAG_*` (tuning)             |    –     | Chunking, retrieval floor, hybrid pool and loop budgets, all with defaults; the full table is in [RAG → Tuning](rag.md#tuning) |
...
| Point storage at real S3/R2 | Set `S3_ENDPOINT`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY`/`S3_BUCKET`; `src/lib/storage/client.ts` stays unmodified either way   |
...
| Go fully offline            | Point `RAG_LLM_BASE_URL` at Ollama/llama.cpp; the embedding model must emit 2048 dims, and the planner must emit native tool calls |
| Tune the agentic loop       | `RAG_MAX_SEARCHES` / `RAG_MAX_LOOP_MS` / `RAG_MAX_LOOP_TOKENS` / `RAG_AGENTIC_FLOOR_STEP`. See [RAG → Tuning](rag.md#tuning)       |
