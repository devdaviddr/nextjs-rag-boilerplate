...
| `planner-unavailable` | The planning call threw or returned nothing usable; the loop stops rather than retries, because retrying a planner that just emitted nothing is how a bounded loop becomes unbounded. If that happens before the first search, the original question is searched once first — the fixed path's retrieval — so a planner outage degrades to the fixed pipeline instead of refusing everything |
