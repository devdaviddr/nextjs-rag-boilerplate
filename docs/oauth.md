...
Both providers are opt-in, independently of each other. Each one wires itself up
only when its two environment variables are present. A fork with neither set
behaves exactly like the password-only default. The buttons are never rendered,
because the provider was never added, so there is nothing for CSS to hide.

## Do you need this?

No. Nothing else in the boilerplate depends on it. The knowledge-base and chat
features work the same whichever way a user signed in, because OAuth here is
two extra providers on the same Auth.js instance and the same `users` table
that password sign-in already uses. Roles, sessions and every permission check
downstream are provider-agnostic.
...
Turning it on takes about ten minutes per provider. You need a free account
with the provider, two environment variables and a restart. There is no code
change and no migration, since the `users` and `accounts` tables are already there
...
Every OAuth provider needs to know where to send the user back to. That
...
defaults to `http://localhost:3000` in local development. On your laptop the
GitHub callback is therefore
...
`AUTH_TRUST_HOST="true"` so the forwarded host is honoured. If `AUTH_URL` is
unset in production, the redirect can land on the wrong origin.

### 2. Register the app with GitHub

1. Go to GitHub → Settings → Developer settings → OAuth Apps → New OAuth App.
2. Set Homepage URL to your `APP_URL` and Authorization callback URL to
   `{APP_URL}/api/auth/callback/github`.
3. Create it, copy the Client ID into `AUTH_GITHUB_ID`, then generate a client
   secret and put it in `AUTH_GITHUB_SECRET`.

### 3. Register the app with Google

1. Go to Google Cloud Console → APIs & Services → Credentials → Create
   credentials → OAuth client ID (configure the consent screen first if
   prompted).
2. Set Application type to Web application.
3. Set Authorized redirect URI to `{APP_URL}/api/auth/callback/google`.
4. Copy the Client ID into `AUTH_GOOGLE_ID` and the Client secret into
   `AUTH_GOOGLE_SECRET`.
...
without a role, and the sign-in still succeeds.
...
Not automatically. Auto-linking on a matching email is off
(`allowDangerousEmailAccountLinking: false`, set explicitly instead of left to
...
To link, sign in the way you already can, then go to Settings → Connected
accounts → Link. From then on either route signs you into the same account.

Unlinking is blocked server-side when it would leave you with no way back in,
meaning no password and no other linked provider. You get an error instead of a
locked account:
...
whenever the email addresses matched, anyone who could get a provider to issue
a token for `sam@example.com` would land inside Sam's existing account. An
explicit link is safe because GitHub and Google both verify email ownership
before issuing a token, so once a signed-in user chooses to attach a provider,
the attachment is trustworthy. The risk comes from linking without asking.

Sessions stay JWT even though an adapter is present. The Drizzle adapter
persists OAuth users and their `accounts` rows. Auth.js would normally flip
...
validated at boot, and are never logged. The login page only receives booleans
(`isGithubConfigured()` and `isGoogleConfigured()`), which is also why an
unconfigured provider renders nothing at all.

## Troubleshooting

If there is no "Continue with …" button on the login page, the provider was
not added. That only happens when both of its variables are set and non-empty.
Check for a typo in the variable name, confirm the values are in the `.env` the
app loads, and restart, because provider assembly happens at startup, not per
request.

If the provider rejects the redirect ("redirect_uri mismatch"), the callback
...
OAuth app for local development instead of editing the production one.

"An account with this email already exists." is the deliberate refusal
described above. Sign in with the existing method, then link the provider from
Settings.

If unlinking is refused, you are trying to remove your only way in. Set a
password or link a second provider first.

## Related

- [Features → Access control (RBAC)](features.md#access-control-rbac): what the
  bootstrap `admin` role unlocks.
- [Spec 0010: OAuth providers](../specs/0010-oauth-providers.md)

Next: [Email](email.md), the other half of account handling. It covers
verification links and password reset, which OAuth sits alongside.
