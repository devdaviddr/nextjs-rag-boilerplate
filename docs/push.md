...
There is no Firebase, no OneSignal, no third-party SDK and no per-message
cost. Your server identifies itself to the push services with a keypair you
generate yourself (a scheme called **VAPID**, Voluntary Application Server
Identification) and talks to them directly through the [`web-push`](https://github.com/web-push-libs/web-push)
...
is elsewhere and they would want to know, such as a slow document finishing
processing, a new user registering, or a document shared with them.

Turning it on takes two minutes, one command and three environment variables.
You need no account with anybody, and there is no ongoing cost. The real
prerequisite is the one below.

## Requirements

Web Push needs HTTPS and an installed service worker. The service worker
receives the message and shows the notification, and it registers in production
builds only, so **a `pnpm dev` session can never receive a push**, no matter how
the keys are set.
...
Do this once per deployment. Keep the output somewhere safe: browsers subscribe against
...
configuration stays off instead of half-working. `VAPID_SUBJECT` is how a push
...
Each browser on each device subscribes separately, and one user can have many
...
3. Trigger the worked example: register a second user from another browser or a
   ...
   your admin device gets a notification. Click it, and an existing tab on the
   target URL is focused or a new one opens.

That worked example exists so the send path is proven end to end instead of
shipped as an untested primitive.
...
There are three moving parts: subscribe, send and receive.

To subscribe, the Settings panel
...
instead of erroring. Disabling calls `unsubscribe()` and deletes the row.

To send, `sendPushNotification` looks up every subscription for the user and
...
On the receiving side, the service worker (`public/sw.js`) handles the `push` event by
...
a malformed or absent body still shows something instead of throwing inside the
worker.

## Security and privacy

- **Payloads can appear on a lock screen.** Anyone holding the device can read
  the `title` and `body` without unlocking it. Put a subject line there, such
  as "Document ready", and keep the document's text out.
- The private key never leaves the server. The browser only receives the public
  key. Treat `VAPID_PRIVATE_KEY` exactly as you treat `AUTH_SECRET`: `.env`
...
- Subscriptions are per-user and ownership-checked. A delete only matches a
  ...
  remove another's. This is the same ownership pattern as
...
- An endpoint can change hands. Two people using one browser profile produce
  ...
  recently. This is deliberate, because otherwise one person's notifications
  would go to the other's session.

## Troubleshooting

If there is no Notifications card in Settings, one of the three `VAPID_*` variables is
missing or empty. All three are required, and the app reads them at startup.

If the card says push is not supported, either the browser lacks
`PushManager` or there is no service worker. The missing worker is the usual
cause, because it only registers in production builds. Run `pnpm build && pnpm start`.

If Enable fails with "Could not enable notifications on this device.", permission
...
If notifications stopped arriving on one device, the subscription probably
expired and was auto-pruned after a 404 or 410 from the push service. Toggle the
Settings switch off and on to resubscribe.

If nothing arrives anywhere after changing keys, it is because existing
subscriptions were created against the old public key. Users have to toggle
Settings off and on to subscribe again, which is why the keypair is generated
once and kept.
...
Next: [Self-hosting](self-hosting.md), on running the whole thing on your own
